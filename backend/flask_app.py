...
def create_app(test_config: dict | None = None) -> Flask:
    ...
    def csrf_token():
        """
        Return a CSRF token tied to this session.
        The frontend fetches this on every auth page load and sends it back
        as the X-CSRF-Token header on mutating requests.
        """
    ...
    def _check_csrf():
        ...
        Returns (None, None) on success, or (response, status) on failure.
        """
        # Allow GET/OPTIONS through
        if request.method in ('GET', 'OPTIONS', 'HEAD'):
            return None, None

        # Skip CSRF for the token-endpoint itself and health check
    ...
    def login():
        ...
        try:
            ...
            # Constant-time failure — no difference between "wrong user" and "wrong pw"
    ...
    def _require_auth():
        """Return (user, None) if authenticated, or (None, error_response)."""
        if not DB_AVAILABLE:
    ...
    def change_password():
        ...
        user.set_password(new_password)
        db.session.commit()
        # Invalidate session after password change for security
        session.clear()
    ...
    def serve_uploaded_file(filename):
        upload_folder = os.path.join(
            os.path.dirname(__file__), '..', 'frontend', 'assets', 'uploads'
        )
        return send_from_directory(upload_folder, filename)

    # ══════════════════════════════════════════════════════════════════════════
    # SIMPLE ENCRYPT / DECRYPT  (password-based — used by encrypt.js/decrypt.js)
    # ══════════════════════════════════════════════════════════════════════════

    @app.route('/api/encrypt', methods=['POST', 'OPTIONS'])
    def encrypt_message():
        """
        Encrypt a message with a user-supplied password.
        The GhostChat class handles AES-256 + emoji obfuscation.
        """
        if request.method == 'OPTIONS':
            return '', 200
        ...
        try:
            ...
            emoji_output = ghost.send_message(message, use_decoy_emojis=use_decoys)

            return jsonify({
                'success':       True,
                'emoji_message': emoji_output,
                'emoji_count':   len(emoji_output),
    ...
    def decrypt_message():
        """
        Decrypt an emoji-obfuscated message using the provided password.
        """
        if request.method == 'OPTIONS':
            return '', 200
    ...
    def save_integration(service):
        ...
        data = request.get_json() or {}
        item = IntegrationToken.query.filter_by(
    ...
    ALLOWED_EXTENSIONS = (
        ...
        '.gif', '.svg', '.ico', '.json', '.woff', '.woff2', '.ttf', '.webp',
    )
